  ...
  return (
    <div className="space-y-6 pb-20">
      <div className="flex justify-between items-center">
        ...
        <button onClick={() => openModal()} className="bg-indigo-600 text-white px-6 py-3 rounded-2xl font-bold flex items-center gap-2 shadow-lg">
          <Plus size={20} /> Nuova
        </button>
      </div>
            ...
            return (
              <div key={offer.id} className="bg-white dark:bg-gray-800 p-6 rounded-[2rem] shadow-sm border border-gray-50 dark:border-gray-700">
                ...
                <div className="flex justify-end gap-2 mt-6">
                  <button onClick={() => openModal(offer)} className="p-2 rounded-xl bg-blue-50 text-blue-600 hover:bg-blue-100"><Edit2 size={16} /></button>
                  <button onClick={() => removeOffer(offer.id)} className="p-2 rounded-xl bg-red-50 text-red-400 hover:bg-red-100"><Trash2 size={16} /></button>
                </div>
              </div>
            );
          })
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-md">
          <div className="bg-white dark:bg-gray-800 w-full max-w-4xl rounded-[2.5rem] p-8 max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="flex justify-between items-center mb-8">
              <h2 className="text-2xl font-black uppercase dark:text-white">{editingId ? 'Modifica' : 'Nuovo Preventivo'}</h2>
            ...
            <div className="space-y-6">
                ...
                {items.map((item) => (
                  <div key={item.id} className="grid grid-cols-12 gap-3 items-end bg-gray-50 dark:bg-gray-900 p-4 rounded-2xl">
                    <div className="col-span-12 md:col-span-4">
                      <select className="w-full bg-transparent font-bold outline-none text-sm dark:text-white border-b pb-1 mb-2" onChange={(e) => handleProductSelect(item.id, e.target.value)}>
                        <option value="">Catalogo...</option>
                        {Object.values(products).map((p) => <option key={p.id} value={p.id}>{p.code} - {p.description}</option>)}
                      </select>
                      <input type="text" placeholder="Descrizione" className="w-full bg-transparent font-bold outline-none text-xs" value={item.description} onChange={(e) => updateItem(item.id, { description: e.target.value })} />
                    </div>
                    <div className="col-span-4 md:col-span-2">
                      <input type="text" placeholder="Taglie" className="w-full bg-transparent font-bold outline-none text-sm border-b pb-1" value={item.sizes || ''} onChange={(e) => updateItem(item.id, { sizes: e.target.value })} />
                    </div>
                    <div className="col-span-4 md:col-span-1 text-center">
                      <input type="number" className="w-full bg-transparent font-bold outline-none text-sm border-b pb-1" value={item.quantity} onChange={(e) => updateItem(item.id, { quantity: parseInt(e.target.value) || 0 })} />
                    </div>
                    <div className="col-span-4 md:col-span-2 text-center">
                      <input type="number" className="w-full bg-transparent font-bold outline-none text-sm border-b pb-1" value={item.discount} onChange={(e) => updateItem(item.id, { discount: parseFloat(e.target.value) || 0 })} />
                    </div>
                    <div className="col-span-10 md:col-span-2 text-right">
                      <input type="number" className="w-full bg-transparent font-bold outline-none text-sm border-b pb-1 text-right text-indigo-600" value={item.price} onChange={(e) => updateItem(item.id, { price: parseFloat(e.target.value) || 0 })} />
                    </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-indigo-50/50 p-6 rounded-2xl">
                <div>
                  <label className="text-[10px] font-black text-indigo-600 uppercase tracking-widest mb-2 flex items-center gap-1"><Truck size={12}/> Tempi Consegna</label>
                  <input type="text" className="w-full border-2 border-white rounded-xl p-3 bg-white dark:bg-gray-800 dark:text-white font-bold outline-none" value={deliveryTime} onChange={e => setDeliveryTime(e.target.value)} />
                </div>
                <div>
                  <label className="text-[10px] font-black text-indigo-600 uppercase tracking-widest mb-2">Trasporto (€)</label>
                  <input type="number" className="w-full border-2 border-white rounded-xl p-3 bg-white dark:bg-gray-800 dark:text-white font-bold outline-none" value={shippingCost} onChange={e => setShippingCost(Number(e.target.value))} />
                </div>
              </div>

              <div className="pt-6 border-t flex justify-between items-center">
                <div className="text-3xl font-black text-indigo-600">€ {calculateTotal().toLocaleString('it-IT')}</div>
