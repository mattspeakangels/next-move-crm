...
import { Search, PackagePlus, Upload, Tag, ChevronRight, X, Package } from 'lucide-react';
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (!line) continue;
        const values = line.split(/[;,]/); // Supporta virgola e punto e virgola
        if (values.length >= 3) {
          newProducts.push({
            id: `p_${Date.now()}_${i}`,
            code: values[0]?.trim() || '',
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      <div className="relative">
        ...
        <input type="text" placeholder="Cerca per nome o codice articolo..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-white dark:bg-gray-800 border-none rounded-2xl py-4 pl-12 pr-4 shadow-sm outline-none" />
      </div>
        ...
        {filteredProducts.map(product => (
          <div key={product.id} className="bg-white dark:bg-gray-800 p-5 rounded-3xl shadow-sm border border-gray-50 flex items-center justify-between group">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="flex gap-3 mt-1 text-xs font-bold">
                  <span className="text-indigo-600 uppercase">Cod: {product.code}</span>
            ...
            <div className="text-right">
              ...
              <span className="text-[10px] text-gray-400 uppercase font-bold">Prezzo Listino</span>
            </div>
          </div>
        ))}
      </div>
      
      {/* Modal semplificato per aggiunta a mano se serve */}
      {showModal && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
           <div className="bg-white dark:bg-gray-800 w-full max-w-md rounded-[2.5rem] p-8">
              ...
              <div className="space-y-4">
                <input placeholder="Codice Articolo" className="w-full border-2 border-gray-100 rounded-2xl p-3" onChange={e => setFormData({...formData, code: e.target.value})} />
                <input placeholder="Nome Prodotto" className="w-full border-2 border-gray-100 rounded-2xl p-3" onChange={e => setFormData({...formData, name: e.target.value})} />
                <input placeholder="Categoria" className="w-full border-2 border-gray-100 rounded-2xl p-3" onChange={e => setFormData({...formData, category: e.target.value})} />
