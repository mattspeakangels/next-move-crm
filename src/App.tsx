...
function AppContent() {
  ...
  const navItems = [
    ...
    { id: 'map' as NavView, icon: Map, label: 'Radar Mappa' },
  ...
  return (
    <div className={`min-h-screen ${theme === 'dark' ? 'dark bg-gray-900' : 'bg-gray-50'}`}>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-800 border-t p-3 flex justify-around z-30">
        {navItems.slice(0, 5).map(({ id, icon: Icon }) => (
          <button key={id} onClick={() => setCurrentView(id)} className={`p-3 rounded-2xl ${currentView === id ? 'text-indigo-600' : 'text-gray-400'}`}><Icon size={24} /></button>
        ))}
      </nav>
    </div>
  );
